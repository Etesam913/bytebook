...
export function LinkedMentions({ filePath }: { filePath: FilePath }) {
  ...
  return (
    <section className="group mt-2 pt-1 border-t border-gray-200 dark:border-gray-600">
      <Disclosure isExpanded={isExpanded} onExpandedChange={setIsExpanded}>
        ...
        <DisclosurePanel className="h-(--disclosure-panel-height) overflow-clip text-xs motion-safe:transition-[height] motion-safe:duration-200">
          {isError && (
            <p className="flex items-center gap-1.5 py-1 pl-5 text-red-500 dark:text-red-400">
              <TriangleWarning width="0.875rem" height="0.875rem" />
              Failed to load linked mentions
            </p>
          )}
          {isLoading ? (
            <div className="flex items-center gap-2 py-1 pl-5 text-zinc-500 dark:text-zinc-300">
              <Loader width="0.875rem" height="0.875rem" /> Loading
            </div>
          ) : mentions.length === 0 ? (
            <section className="py-1 pl-5 gap-1.5 flex flex-col text-zinc-500 dark:text-zinc-300">
            ...
            <Virtuoso
              style={{ height: Math.min(mentions.length * 26, 260) }}
              ...
              itemContent={(_, mention) => {
                ...
                return (
                  <Link
                    ...
                    className="group/mention flex w-fit min-w-0 items-center gap-1 py-1 pl-5 pr-3 overflow-hidden rounded-md text-zinc-800 hover:text-(--accent-color) dark:text-zinc-200"
                  >
                    {folderPathLabel && (
