...
- Imports are relative (`../utils/...`) — there is no path alias.
...
- Styling is Tailwind v4 utilities with `dark:` variants. Conditional classes go through `cn()` from `utils/string-formatting.ts`. Reserve inline `style={{}}` for dynamic/measured values.
- Paths: use `createFilePath()` / `createFolderPath()` from `utils/path.ts` (typed `FilePath` / `FolderPath`). `LocalFilePath` is legacy — don't use it in new code.
