...
func TestEnsureIdInFrontmatter(t *testing.T) {
	t.Run("should return existing id when it exists", func(t *testing.T) {
		markdown := `---
id: existing-id-123
title: Test Document
---
...
`
		...
		result, id := EnsureIdInFrontmatter(markdown)

		assert.Equal(t, "existing-id-123", id)
		assert.Equal(t, markdown, result) // Should be unchanged
	})

	t.Run("should add id to existing frontmatter without id", func(t *testing.T) {
		markdown := `---
title: Test Document
author: Test Author
---
...
`
		...
		// Should contain the id in frontmatter
		resultId, exists := GetIdFromFrontmatter(result)
		assert.True(t, exists)
	...
	t.Run("should create frontmatter when none exists", func(t *testing.T) {
		markdown := `# Content
...
`
		...
		// Should preserve content
		assert.Contains(t, result, "# Content")
		assert.Contains(t, result, "This is some content without frontmatter.")
	...
	t.Run("should handle empty markdown", func(t *testing.T) {
		...
		assert.True(t, strings.HasPrefix(result, "---\n"))
		assert.True(t, strings.Contains(result, "id: "+id))
	})

	t.Run("should handle malformed frontmatter", func(t *testing.T) {
		markdown := `---
invalid: yaml: content: [
---
...
`
	...
	t.Run("should preserve complex frontmatter structure", func(t *testing.T) {
		markdown := `---
...
`
		...
		assert.Contains(t, result, "title: Complex Document")
		...
		assert.Contains(t, result, "lastUpdated: 2023-12-01T10:30:00Z")
	...
	t.Run("should generate different ids for different calls", func(t *testing.T) {
		markdown := `# Content
This is some content.
`

		_, id1 := EnsureIdInFrontmatter(markdown)
		_, id2 := EnsureIdInFrontmatter(markdown)
